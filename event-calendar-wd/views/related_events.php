                        ...
                        <li itemscope itemtype="http://schema.org/Event" class="upcoming_events_item">
                            <div class="upcoming_event_container">
                                ...
                                <div class="event-date" itemprop="startDate"
                                     content="<?php echo ECWD::ecwd_date('Y-m-d', strtotime($ecwd_event['from'])) . 'T' . ECWD::ecwd_date('H:i', strtotime($ecwd_event['starttime'])) ?>">
                                         <?php
                                         if (isset($ecwd_event['all_day_event']) && $ecwd_event['all_day_event'] == 1) {
                                             echo ECWD::ecwd_date($date_format, strtotime($ecwd_event['from']));
                                             if ($ecwd_event['to'] && ECWD::ecwd_date($date_format, strtotime($ecwd_event['from'])) !== ECWD::ecwd_date($date_format, strtotime($ecwd_event['to']))) {
                                                 echo ' - ' . ECWD::ecwd_date($date_format, strtotime($ecwd_event['to']));
                                             }
                                             echo ' ' . __('All day', 'event-calendar-wd');
                                         } else {

                                             echo ECWD::ecwd_date($date_format, strtotime($ecwd_event['from'])) . ' ' . ECWD::ecwd_date($time_format, strtotime($ecwd_event['starttime']));

                                             if ($ecwd_event['to']) {
                                                 echo ' - ' . ECWD::ecwd_date($date_format, strtotime($ecwd_event['to'])) . ' ' . ECWD::ecwd_date($time_format, strtotime($ecwd_event['endtime']));
                                             }
