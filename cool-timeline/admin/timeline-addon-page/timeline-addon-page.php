    ...
    class cool_plugins_timeline_addons
    {
         ...
         */
            ...
            function cool_plugins_install(){
                if( isset( $_POST['wp_nonce'] ) && isset( $_POST['nonce_name'] ) && wp_verify_nonce( $_POST['wp_nonce'] , $_POST['nonce_name'] )){
                    require_once CTP_PLUGIN_DIR . 'admin/timeline-addon-page/includes/cool_plugins_downloader.php';
                    $downloader = new cool_plugins_downloader();                  
                    return  $downloader->install( filter_var($_REQUEST['url'], FILTER_SANITIZE_URL), 'install' );
                }else{
                    die('wp nonce verification failed!');
                }
                die();
            ...
            function displayPluginAdminDashboard(){
                ...
                if( !empty( $plugins ) && count( $plugins ) > 0 ){
                    ...
                    if( !empty($this->pro_plugins) && count($this->pro_plugins) >0 ):
                         ...
                         */
                    echo "<div class='plugins-list pro-addons' data-empty-message='No more Pro plugins available at the moment'><h3>Timeline Pro Plugins</h3>";
